...
class SavitzkyGolaySmoothing:
    ...
    def __init__(self, window_length: int = 7, polyorder: int = 3):
        ...
        if window_length <= polyorder:
            raise ValueError(f"Window length ({window_length}) must be greater than polynomial order ({polyorder})")
    ...
    def __call__(self, data) -> np.ndarray:
        ...
        if spectra.ndim == 1:
            ...
            smoothed = np.array([self._smooth_spectrum(spectrum) for spectrum in spectra])
        else:
    ...
    def _smooth_spectrum(self, spectrum: np.ndarray) -> np.ndarray:
        ...
        if len(spectrum) < self.window_length:
            create_logs("smoothing_warning", "SavitzkyGolaySmoothing",
        ...
        try:
            ...
            create_logs("smoothing_error", "SavitzkyGolaySmoothing",
