...
def perform_spectral_comparison(
    ...
    if show_ci:
        ax1.fill_between(wavenumbers, mean1 - ci1, mean1 + ci1, alpha=0.2, color="blue")
        ax1.fill_between(wavenumbers, mean2 - ci2, mean2 + ci2, alpha=0.2, color="red")
...
def perform_peak_analysis(
    ...
    ax1.set_ylabel("Intensity", fontsize=12)
    ax1.set_title(
        "Peak Analysis with Component Assignments", fontsize=14, fontweight="bold"
    )
    ax1.legend(loc="best")
    ax1.grid(True, alpha=0.3)
    ax1.invert_xaxis()
...
def perform_correlation_analysis(
    ...
    ax.set_title(
        f"Correlation Matrix ({method.capitalize()})", fontsize=14, fontweight="bold"
    )
