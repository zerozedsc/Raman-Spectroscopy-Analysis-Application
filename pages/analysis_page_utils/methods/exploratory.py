...
def perform_pca_analysis(
    ...
    if show_loadings:
        ...
        fig_loadings, axes = plt.subplots(
            max_loadings, 1, figsize=(12, 4 * max_loadings)
        )
        ...
        for pc_idx in range(max_loadings):
            ...
            ax.set_title(
                f"PC{pc_idx+1} Loadings (Explained Variance: {explained_var:.2f}%)",
                fontsize=12,
                fontweight="bold",
            )
            ...
            top_indices = np.argsort(abs_loadings)[
                -5:
            ]  # Top 5 peaks (increased from 3)
        ...
        print(
            f"[DEBUG] Loadings figure created successfully with {max_loadings} subplots"
        )
    else:
    ...
    if show_distributions and len(unique_labels) > 1:
        ...
        fig_distributions.suptitle(
            "PC Score Distributions", fontsize=16, fontweight="bold"
        )
        ...
        for idx in range(n_pcs_to_plot):
            ...
            ax.set_title(
                f"PC{pc_idx+1} ({pca.explained_variance_ratio_[pc_idx]*100:.1f}%)",
                fontsize=13,
                fontweight="bold",
            )
...
def create_spectrum_preview_figure(
    ...
    title = (
        "All Spectra (Vertically Stacked)"
        if show_all
        else "Mean Spectra (Vertically Stacked)"
    )
