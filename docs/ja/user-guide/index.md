...
1. **[カスタムパイプライン](#)** - 独自のワークフロー
2. **[バッチ処理](#)** - 大規模データセット
3. **[プログラマティックアクセス](#)** - Pythonスクリプト
4. **[カスタム手法](#)** - 独自のアルゴリズム
...
**[結果のエクスポート](exporting-results.md)**
- Excelへのエクスポート
- 図の保存（PNG, PDF, SVG）
- レポート生成
- バッチエクスポート
...
![基本的な探索的分析ワークフロー：データのインポート → グループ作成 → 前処理適用 → PCA実行 → スコアプロット確認 → 結果エクスポート](../assets/workflows/exploratory-analysis-workflow-ja.png)
...
![グループ比較ワークフロー：ラベル付きデータロード → グループ定義 → 前処理パイプライン → 統計検定選択 → 検定実行 → 有意な波数特定 → レポート生成](../assets/workflows/group-comparison-workflow-ja.png)
...
graph TD
    ...
    E --> F[クロスバリデーション]
    F --> G[テストセットで評価]
![機械学習分類ワークフロー：訓練データロード → 前処理最適化 → データ分割 → モデル選択 → トレーニング → クロスバリデーション → テストセット評価 → モデル保存 → 新データ予測](../assets/workflows/ml-classification-workflow-ja.png)
2. **前処理最適化**（15分）
   ```
   複数のパイプラインを試す:
...
- **[チュートリアル](../getting-started/index.md)** - ステップバイステップガイド
