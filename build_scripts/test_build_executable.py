...
class TestSuite:
    ...
    def run_all(self) -> Tuple[int, int, int]:
        ...
        print(
            f"🧩 Detected Build Mode: {'onefile' if self.is_onefile else 'onedir (or extracted)'}\n"
        )
    ...
    def _test_required_directories(self):
        ...
        try:
            if self.is_onefile:
                ...
                test.details["note"] = (
                    "onedir builds are expected to include assets/_internal/PySide6 folders"
                )
                test.duration = time.time() - start
                self.results.append(test)
                print(test.to_string(self.verbose))
    ...
    def _test_required_assets(self):
        ...
        try:
            if self.is_onefile:
                test.status = "SKIP"
                test.message = (
                    "Onefile build detected; assets are embedded in the executable"
                )
                test.details["note"] = (
                    "This test is only meaningful for onedir distributions"
                )
                test.duration = time.time() - start
                self.results.append(test)
                print(test.to_string(self.verbose))
    ...
    def _test_binaries(self):
        ...
        try:
            if self.is_onefile:
                ...
                test.details["note"] = (
                    "This test expects on-disk DLLs (typical for onedir distributions)"
                )
                test.duration = time.time() - start
                self.results.append(test)
                print(test.to_string(self.verbose))
