...
#### 解析が終わらない / Stop（停止）が効かないように見える

**補足:** 一部の解析はバックグラウンドスレッドで実行されます。停止は安全のため、
「キャンセル要求」→（必要なら）「UIを切り離して結果を無視」という2段階の方式になります。

**対処:**
1. **Stop** を1回押してキャンセル要求を送ります。
2. それでも止まらない場合は **Stop** をもう1回押すと、UIを即座に復帰させ（その解析結果は無視）、アプリ自体は落ちないようにします。

#### Matplotlib の警告「Glyph ... missing from font(s) DejaVu Sans」（日本語表示）

**原因:** Qt側でフォントを読み込んでも、Matplotlib には別途フォント登録が必要です。

**対処:**
1. `assets/fonts/` に `Noto Sans JP.ttf` などのフォントが含まれているか確認してください。
2. プロジェクトルートから起動してパス解決が正しくなるようにしてください。
3. 実行ファイル（portable/installer）で警告が残る場合は、ビルド時に `assets/fonts` を確実に同梱してください。

#### 相関ヒートマップの軸ラベル/目盛りが見切れる、読めない

**対処:**
1. プロット領域を少し大きくしてみてください（リサイズ時に再レイアウトされます）。
2. 変数数が多い場合はダウンサンプリングや目盛り非表示（見やすさ優先）になります。

#### Band Ratio のx軸ラベルが長すぎて重なる

**対処:**
1. ラベルが長い場合は自動でフォントを小さくし、下側マージンも増やすよう調整されています。
2. それでも重なる場合は、データセット名を短くする/表示グループ数を減らしてください。
